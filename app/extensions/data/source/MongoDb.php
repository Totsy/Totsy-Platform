...
class MongoDb extends \lithium\data\source\MongoDb {
	...
	public function create($query, array $options = array()) {
		...
		return $this->_filter(__METHOD__, $params, function($self, $params) use ($_config) {
			...
			$data = $self->invokeMethod('_toMongoDate', array($query->data()));
	...
	public function update($query, array $options = array()) {
		...
		return $this->_filter(__METHOD__, $params, function($self, $params) use ($_config) {
			...
			$update = $self->invokeMethod('_toMongoDate', array($args['data']));
	...
	protected function _toMongoDate($data) {
		foreach ($data as $key => $value) {
			if (is_array($value)) {
				if (array_key_exists('sec', $value) && array_key_exists('usec', $value)) {
					$data[$key] = new MongoDate($value['sec'], $value['usec']);
				}
			}
		}
		return $data;
