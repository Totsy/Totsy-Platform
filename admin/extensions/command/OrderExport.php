...
class OrderExport extends Base {
	...
	protected function _orderGenerator() {
		...
		$orders = Order::findUncanceledByEvent($this->orderEvents);
		...
		/**
		 * @todo This whole if/else block should be extracted out into a protected method.
		 */
		...
		foreach ($orders as $order) {
			/**
			 * @todo: This should be replaced with a map/reduce call outside the loop that
			 * aggregates counts by order ID.
			 */
			$conditions = array('Customer PO #' => array(
				'$in' => array((string) $order['_id'], $order['_id'])
			));
			$processCheck = ProcessedOrder::count(compact('conditions'));
			++$ecounter;
			...
			$user = $users[(string) $order['user_id']];
			$itemIds = array_filter(array_map(
				function($i) {
					if (empty($i['cancel']) || $i['cancel'] != true) {
						return null;
					}
					return new MongoId((string) $i['item_id']);
				},
				$items
			));

			$orderItems = array();
			$orderItems = $itemCollection->find(array('_id' => array('$in' => $itemIds)));

			foreach ($orderItemList as $item) {
				$orderItems[(string) $item['_id']] = $item;
			}
			...
			foreach ($items as $item) {
				...
				$orderFile[$inc]['Order Creation Date'] = date(
					"m/d/Y", str_replace("0.00000000 ", "", $order['date_created'])
				);
				$orderFile[$inc]['Promised Ship-by Date'] = date(
					"m/d/Y", str_replace("0.00000000 ", "", $order['ship_date'])
				);
				...
				$processedOrder = ProcessedOrder::collection();
		...
		$this->summary['order']['filename'] = $filename;
		$this->log(
			"{$handle} was created total of {$totalOrders} orders generated with {$inc} lines"
		);
		return true;
	}
	...
	protected function _itemGenerator() {
		...
		$eventIds = array_unique(array_merge(
			$this->orderEvents, $this->poEvents, $this->addEvents
		));
		...
		if (!$eventIds) {
			...
			$this->log(
				"There were {$count} items generated and saved to the item master and {$handle}"
			);
			return true;
		}

		$events = array();
		$eventList = Event::find('first', array('conditions' => array('_id' => $eventIds)));
		$productHeading = ProcessedOrder::$_productHeading;

		foreach ($eventList as $event) {
			$events[(string) $event->_id] = $event;
		}
		...
		foreach ($eventIds as $eventId) {
			...
			foreach ($eventItems as $eventItem) {
				foreach ($eventItem['details'] as $key => $value) {
					 ...
					 * If not in item master create one
					 */
					list($sku, $eventItem) = $this->_makeSku($eventItem, $key, $description);
					...
					if ($itemMasterCheck == 0) {
						...
						$fields[$inc]['WhsInsValue (Cost)'] = number_format(
							$eventItem['sale_whol'], 2
						);
						$fields[$inc]['Description for Customs'] = $eventItem['category'] ?: '';
						$fields[$inc]['ShipInsValue'] = number_format(
							$eventItem['orig_whol'], 2
						);
						...
						if ((int) $eventItem['product_weight'] > 0) {
							$fields[$inc]['UOM1_Weight'] = number_format(
								$eventItem['product_weight'], 2
							);
						}
						$fields[$inc]['Style'] = substr($eventItem['vendor_style'], 0, 15);
						$fields[$inc] = array_merge($productHeading, $fields[$inc]);
						$productFile[$inc] = $this->sortArrayByArray(
							$fields[$inc], $productHeading
						);
					}
	...
	protected function _makeSku($eventItem, $key, $description) {
		if (array_key_exists('sku_details', $eventItem)) {
			return array($eventItem['sku_details'][$key], $eventItem);
		}
		if ($sku = $this->findSku(String::asciiClean($description), $key)) {
			return array($sku, $eventItem);
		}
		$makeSku = new MakeSku();
				...
				foreach ($eventItem['details'] as $key => $value) {
					...
					if ($orders) {
						foreach ($orders as $order) {
							...
							$purchaseOrder[$inc]['WhsInsValue (Cost)'] = number_format(
								$eventItem['sale_whol'], 2
							);
							$purchaseOrder[$inc]['Description for Customs'] = $eventItem['category'] ?: "";
							$purchaseOrder[$inc]['ShipInsValue'] = number_format(
								$eventItem['orig_whol'], 2
							);
							...
							if ((int) $eventItem['product_weight'] > 0) {
								$purchaseOrder[$inc]['UOM1_Weight'] = number_format(
									$eventItem['product_weight'], 2
								);
							}

									$purchaseOrder[$inc] = $this->sortArrayByArray(
										$purchaseOrder[$inc], $purchaseHeading
									);
								}
							}
						}
	...
	protected function findSku($description, $size) {
		...
		return ($item_master) ? $item_master['SKU'] : false;
	}
	...
	**/
	...
	protected function _getOrderItems($eventId = null) {
		...
		$items = Item::find('all', array(
			'conditions' => array('event' => array('$in' => array($eventId))
		)));
		return $items->data();
	}
}
