...
class EventsController extends BaseController {
	...
	public function upload($_id = null) {
		...
		if ($_FILES['upload_file']['error'] == 0 && $_FILES['upload_file']['size'] > 0) {
			$parseItems = Upload::parseItems($_FILES, $event->_id, $enableFinalsale, $enableItems);
	...
	public function edit($_id = null) {
		...
		if (!empty($this->request->data)) {
			...
			$enableItems = $this->request->data['enable_items'];
			if ($_FILES['upload_file']['error'] == 0 && $_FILES['upload_file']['size'] > 0) {
				if (is_array(Upload::parseItems($_FILES, $event->_id, $enableItems))) {
					unset($this->request->data['upload_file']);
					$eventItems = Item::find('all', array('conditions' => array('event' => array($_id))));
					if (!empty($eventItems)) {
						foreach ($eventItems as $item) {
					...
					//parse items xls gave an error
					//return the error to the page
					unset($this->request->data['upload_file']);
					$upload_error = "<br><br><font color=ff0000>error city</font><br><br>"; 
